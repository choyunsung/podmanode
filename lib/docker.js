...
var Docker = function(opts) {
  ...
  // Auto-detect Podman socket if no options provided
  if (!opts || (!opts.socketPath && !opts.host)) {
    var fs = require('fs');
    var os = require('os');

    // Try Podman rootless socket first
    var podmanSocket = '/run/user/' + os.userInfo().uid + '/podman/podman.sock';
    if (fs.existsSync(podmanSocket)) {
      opts = opts || {};
      opts.socketPath = podmanSocket;
    } else {
      // Try Podman rootful socket
      var podmanRootSocket = '/run/podman/podman.sock';
      if (fs.existsSync(podmanRootSocket)) {
        opts = opts || {};
        opts.socketPath = podmanRootSocket;
      }
      // If neither Podman socket exists, fall back to Docker socket (default in Modem)
    }
  }
...
/**
 * Fetches a Pod by name or ID (Podman-specific feature)
 * NOTE: This will only work when connected to Podman, not Docker
 * @param {String} id Pod's name or ID
 */
Docker.prototype.getPod = function(id) {
  return new Pod(this.modem, id);
};
...
/**
 * Lists pods (Podman-specific feature)
 * NOTE: This will only work when connected to Podman, not Docker
 * @param {Object}   opts     Options (optional)
 * @param {Function} callback Callback
 */
Docker.prototype.listPods = function(opts, callback) {
  var self = this;
  var args = util.processArgs(opts, callback);
...
/**
 * Creates a new pod (Podman-specific feature)
 * NOTE: This will only work when connected to Podman, not Docker
 * @param {Object}   opts     Create options
 * @param {Function} callback Callback
 */
Docker.prototype.createPod = function(opts, callback) {
  var self = this;
  var args = util.processArgs(opts, callback);
  var optsf = {
  ...
  if (args.callback === undefined) {
    return new this.modem.Promise(function(resolve, reject) {
      self.modem.dial(optsf, function(err, data) {
        ...
        resolve(self.getPod(data.Id));
      });
    });
  } else {
    this.modem.dial(optsf, function(err, data) {
      if (err) return args.callback(err, data);
      args.callback(err, self.getPod(data.Id));
    });
  }
};

/**
 * Prune pods (Podman-specific feature)
 * NOTE: This will only work when connected to Podman, not Docker
 * @param {Object}   opts     Options (optional)
 * @param {Function} callback Callback
 */
Docker.prototype.prunePods = function(opts, callback) {
  var self = this;
  var args = util.processArgs(opts, callback);
...
/**
 * Init swarm.
 * WARNING: Swarm is not supported in Podman. This will fail when connected to Podman.
 *
 * @param {object} opts
 ...
 * Inspect a Swarm.
 * WARNING: Swarm is not supported in Podman. This will fail when connected to Podman.
 * Note: This method is not documented in the Docker API
 *
 * @param {Object}   opts     Options (optional)
 * @param {Function} callback Callback
 */
